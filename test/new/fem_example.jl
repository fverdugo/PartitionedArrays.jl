...
function setup_params()
    ...
    (;parts_per_dir,length_per_dir,cells_per_dir,nodes_per_dir,u,f)
end
...
function setup_space(grid)
    ...
    n_own_dofs, (;local_node_to_dof,local_dof_to_owner,permutation)
end
        ...
        for (element_node,cartesian_element_node) in enumerate(grid.linear_to_cartesian_element_node)
            ...
            if local_dof > 0
                ...
                if own_dof > 0
                    ...
                    local_cell_to_global_dofs[local_cell][element_node] = global_dof
                end
            end
        end
    end
    local_cell_to_global_dofs
end

function finish_dofs(space,grid,local_cell_to_global_dofs,tentative_dof_indices)
    ...
    for cartesian_local_cell in grid.linear_to_cartesian_local_cell
        ...
        for (element_node,cartesian_element_node) in enumerate(grid.linear_to_cartesian_element_node)
            ...
            if local_dof > 0
                ...
                if ghost_dof < 0
                    global_dof = local_cell_to_global_dofs[local_cell][element_node]
...
function fem_example(distribute)
    ...
    dof_indices = map(finish_dofs,space,grid,local_cell_to_global_dofs,tentative_dofs.indices)
    ...
    fe = fill(h^2*0.25,4)
    linear_to_cartesian_global_cell = CartesianIndices(cells_per_dir)
    linear_to_cartesian_global_node = CartesianIndices(nodes_per_dir)
    cartesian_to_linear_global_node = LinearIndices(linear_to_cartesian_global_node)
    linear_to_cartesian_element_node = CartesianIndices(ntuple(i->2,Val(D)))
    cartesian_to_linear_element_node = LinearIndices(linear_to_cartesian_element_node)
    cartesian_offset = CartesianIndex(ntuple(i->1,Val(D)))
    ...
    grid = map(cells.indices) do cell_indices
        rank = get_owner(cell_indices)
        local_to_global_cell = get_local_to_global(cell_indices)
        local_cell_to_owner = get_local_to_owner(cell_indices)
        first_global_cell = first(local_to_global_cell)
    ...
    space = map(grid) do grid
        local_node_to_dof = ones(Int,length(grid.local_to_global_cartesian_node))
        for cartesian_local_cell in grid.linear_to_cartesian_local_cell
            cartesian_global_cell = grid.local_to_global_cartesian_cell[cartesian_local_cell]
            for cartesian_element_node in linear_to_cartesian_element_node
                cartesian_global_node = cartesian_global_cell + (cartesian_element_node - cartesian_offset)
                boundary = any(map(is_boundary_node,Tuple(cartesian_global_node),nodes_per_dir))
                if boundary
                    cartesian_local_node = cartesian_local_cell + (cartesian_element_node - cartesian_offset)
                    local_node = grid.cartesian_to_linear_local_node[cartesian_local_node]
                    local_node_to_dof[local_node] = 0
                end
            end
        ...
        n_local_dofs = length(local_dof_to_node)
        local_node_to_dof[local_dof_to_node] = 1:n_local_dofs
        local_dof_to_owner = zeros(Int,n_local_dofs)
        for cartesian_local_cell in grid.linear_to_cartesian_local_cell
            local_cell = grid.cartesian_to_linear_local_cell[cartesian_local_cell]
            owner = local_cell_to_owner[local_cell]
            for cartesian_element_node in linear_to_cartesian_element_node
        ...
        (;n_own_dofs,local_node_to_dof,local_dof_to_owner,local_to_global_dof)
    end
    ...
    # Count the number of own dofs in each subdomain
    n_own_dofs,local_node_to_dof,local_dof_to_owner = map(cells.indices) do cell_indices
        rank = get_owner(cell_indices)
        local_to_global_cell = get_local_to_global(cell_indices)
        local_cell_to_owner = get_local_to_owner(cell_indices)
        first_global_cell = first(local_to_global_cell)
        ...
        cartesian_to_linear_local_node = LinearIndices(local_to_global_cartesian_node)
        cartesian_to_linear_local_cell = LinearIndices(local_to_global_cartesian_cell)
        local_node_to_dof = ones(Int,length(local_to_global_cartesian_node))
        for cartesian_local_cell in linear_to_cartesian_local_cell
            cartesian_global_cell = local_to_global_cartesian_cell[cartesian_local_cell]
            for cartesian_element_node in linear_to_cartesian_element_node
                cartesian_global_node = cartesian_global_cell + (cartesian_element_node - cartesian_offset)
                boundary = any(map(is_boundary_node,Tuple(cartesian_global_node),nodes_per_dir))
                if boundary
                    cartesian_local_node = cartesian_local_cell + (cartesian_element_node - cartesian_offset)
                    local_node = cartesian_to_linear_local_node[cartesian_local_node]
                    local_node_to_dof[local_node] = 0
                end
            end
        end
        ...
        local_dof_to_owner = zeros(Int,n_local_dofs)
        for cartesian_local_cell in linear_to_cartesian_local_cell
            local_cell = cartesian_to_linear_local_cell[cartesian_local_cell]
            owner = local_cell_to_owner[local_cell]
            for cartesian_element_node in linear_to_cartesian_element_node
                cartesian_local_node = cartesian_local_cell + (cartesian_element_node - cartesian_offset)
                local_node = cartesian_to_linear_local_node[cartesian_local_node]
        ...
        n_own_dofs = count(i->i==rank,local_dof_to_owner)
        n_own_dofs,local_node_to_dof,local_dof_to_owner
    end |> unpack

    n_global_dofs = sum(n_own_dofs)
    tentative_dofs = variable_partition(n_own_dofs,n_global_dofs)

    local_cell_to_global_dofs = map(cells.indices,tentative_dofs.indices,local_node_to_dof,local_dof_to_owner) do cell_indices,tentative_dof_indices,local_node_to_dof,local_dof_to_owner
        rank = get_owner(cell_indices)
        local_to_global_cell = get_local_to_global(cell_indices)
        local_cell_to_owner = get_local_to_owner(cell_indices)
        own_to_global_dof = get_own_to_global(tentative_dof_indices)
        ...
        cartesian_to_linear_local_node = LinearIndices(local_to_global_cartesian_node)
        cartesian_to_linear_local_cell = LinearIndices(local_to_global_cartesian_cell)
        n_element_nodes = length(linear_to_cartesian_element_node)
        n_local_cells = length(local_to_global_cell)
        ptrs = zeros(Int,n_local_cells+1)
        ptrs[2:end] .= n_element_nodes
        length_to_ptrs!(ptrs)
        data = zeros(Int,n_element_nodes*n_local_cells)
        local_cell_to_global_dofs = JaggedArray(data,ptrs)
        n_local_dofs = length(local_dof_to_owner)
        local_dof_to_own_dof = zeros(Int,n_local_dofs)
        own_dof_to_local_dof = findall(i->i==rank,local_dof_to_owner)
        n_own_dofs = length(own_dof_to_local_dof)
        local_dof_to_own_dof[own_dof_to_local_dof] = 1:n_own_dofs
        for cartesian_local_cell in linear_to_cartesian_local_cell
            local_cell = cartesian_to_linear_local_cell[cartesian_local_cell]
            for (element_node,cartesian_element_node) in enumerate(linear_to_cartesian_element_node)
                cartesian_local_node = cartesian_local_cell + (cartesian_element_node - cartesian_offset)
                local_node = cartesian_to_linear_local_node[cartesian_local_node]
                local_dof = local_node_to_dof[local_node]
                if local_dof > 0
                    own_dof = local_dof_to_own_dof[local_dof]
                    if own_dof > 0
                        global_dof = own_dof + dof_offset
                        local_cell_to_global_dofs[local_cell][element_node] = global_dof
                    end
        ...
        display(local_cell_to_global_dofs)
        local_cell_to_global_dofs
    end
    cell_to_global_dofs = PVector(local_cell_to_global_dofs,cells)
    ...
    # Find how many nodes per subdomain
    n_own_nodes = map(get_own_to_global(cells)) do own_to_global_cell
        first_cartesian_cell = linear_to_cartesian_global_cell[own_to_global_cell[1]]
        last_cartesian_cell = linear_to_cartesian_global_cell[own_to_global_cell[end]]
        n_own_nodes = prod(Tuple(last_cartesian_cell - first_cartesian_cell + 2*cartesian_offset))
        n_own_nodes
    end
    display(n_own_nodes)
    aaaaa
    ...
    I,J,V = map(get_own_to_global(cells)) do own_to_global_cell
        I = Int[]
        J = Int[]
        V = Float64[]
        for (own_cell,global_cell) in enumerate(own_to_global_cell)
            cartesian_global_cell = linear_to_cartesian_global_cell[global_cell]
            for cartesian_element_row in linear_to_cartesian_element_node
                cartesian_global_row = cartesian_global_cell + (cartesian_element_row - cartesian_offset)
                global_row = cartesian_to_linear_global_node[cartesian_global_row]
                element_row = cartesian_to_linear_element_node[cartesian_element_row]
                boundary = any(map(is_boundary_node,Tuple(cartesian_global_row),nodes_per_dir))
                if boundary
                    push!(I,global_row)
                    push!(J,global_row)
                    push!(V,1.0)
                else
                    for cartesian_element_col in linear_to_cartesian_element_node
                        cartesian_global_col = cartesian_global_cell + (cartesian_element_col - cartesian_offset)
                        global_col = cartesian_to_linear_global_node[cartesian_global_col]
                        element_col = cartesian_to_linear_element_node[cartesian_element_col]
                        push!(I,global_row)
                        push!(J,global_col)
    ...
    # Create tentative partition for rows and cols
    rows = uniform_partition(rank,parts_per_dir,nodes_per_dir)
    cols = rows

    # Start assembling the matrix
    t = psparse!(I,J,V,rows,cols)

    # Meanwhile, we can start computing the rhs values
    I,V = map(get_own_to_global(cells)) do own_to_global_cell
        I = Int[]
        V = Float64[]
        for (own_cell,global_cell) in enumerate(own_to_global_cell)
            cartesian_global_cell = linear_to_cartesian_global_cell[global_cell]
            for cartesian_element_row in linear_to_cartesian_element_node
                ...
                boundary = any(map(is_boundary_node,Tuple(cartesian_global_row),nodes_per_dir))
                if boundary
                    node_coordinates = Tuple(cartesian_element_row - cartesian_offset) .*h
                    push!(I,global_row)
                    push!(V,u(node_coordinates))
                else
                    push!(I,global_row)
                    push!(V,fe[element_row])
                end
            end
        end
        I,V
    ...
    # Wait for the matrix to be ready and fetch it
    A = fetch(t)

    # Now, we can start by assembling the vector
    # Note that we do not need to discover the local row ids
    # since they are the same as for the matrix
    t = pvector!(I,V,A.rows,discover_rows=false)

    # Meanwhile, we can fill the exact solution and initial guess
    x = pzeros(A.cols)
    x̂ = similar(x)
    map(get_own_values(x),get_own_values(x̂),get_own_to_global(A.cols)) do own_x, own_x̂, own_to_global_col
        for (own_col,global_col) in enumerate(own_to_global_col)
            cartesian_global_col = linear_to_cartesian_global_node[global_col]
            node_coordinates = Tuple(cartesian_global_col - cartesian_offset) .*h
            own_x̂[own_col] = u(node_coordinates)
            boundary = any(map(is_boundary_node,Tuple(cartesian_global_col),nodes_per_dir))
            if boundary
                own_x[own_col] = own_x̂[own_col]
            end
        end
    end

    # Wait for the rhs to be ready and fetch it
    b = fetch(t)

    display(A.rows.indices)
